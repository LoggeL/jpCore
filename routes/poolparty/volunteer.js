module.exports = (app, db) => {
    ...
    app.post('/api/private/poolparty/volunteer', async (req, res) => {
        ...
        try {
            ...
            if (!registered) return res.send(400).json({ error: 'Not yet registered' })

            const alreadyVolunteer = await db('volunteer').where('account_id', userID)
            if (alreadyVolunteer) return res.send(400).json({ error: 'Already registered as volunteer' })
    ...
    app.delete('/api/private/poolparty/volunteer', async (req, res) => {
        ...
        try {
            const response = db('volunteer').where('account_id', account_id).del()
